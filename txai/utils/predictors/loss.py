...
class ConnectLoss_Mt(nn.Module):
    def __init__(self):
        super(ConnectLoss_Mt, self).__init__()

    def forward(self, logits):               # [T, B, d]
        diff = logits[1:, :, :] - logits[:-1, :, :]      # [T-1, B, d]
        per_sample = diff.pow(2).mean(dim=(0, 2)).sqrt()  # [B]

        #per_sample = diff.abs().mean(dim=(0, 2))      # [B]
