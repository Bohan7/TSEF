...
@torch.no_grad()
def eval_mvts_transformer_asr(test_tuple, model, batch_size = None, auprc = False, auroc = False):
    '''
    Returns f1 score
    '''
    model.eval()

    X, times, y, y_correct = test_tuple
    ...
    predictions = pred.argmax(dim=1).detach().cpu().numpy()
    # asr = (y.cpu().numpy() != predictions).mean()
    # asr = (y_correct.cpu().numpy() != predictions).mean()
    asr = (y.cpu().numpy() == predictions).mean()

    if auprc and auroc:
        return f1, auprc_val, auroc_val, asr
    elif auprc:
        return f1, auprc_val, asr
    elif auroc:
        return f1, auroc_val, asr
    else:
        return f1, asr
