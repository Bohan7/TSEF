...
def get_explainer(key, args, device = None):
    ...
    if key == 'fit':
        ...
        def explainer(model, X_batch, times_batch, y_batch):
            # Batched IG computation
            # X_batch: [B, T, d], times_batch: [B, T], y_batch: [B]
            from captum.attr import IntegratedGradients
            
            IG = IntegratedGradients(model)
            
            # Compute IG for entire batch
            attr = IG.attribute(
                X_batch.transpose(0, 1),
                target=y_batch,
                additional_forward_args=(times_batch.transpose(0,1), None, True),
                n_steps=1,
